    ...
    <div class="row justify-content-center mt-3">
        <div class="col-lg-6">
            <div class="card p-3 shadow position-relative">
                ...
                <div class=" my-3">
                    <h1 class="fs-2 text-center">Update {{ $employee->first_name }}</h1>
                </div>
                <form method="POST" action="{{ route('employees.update' , $employee->id) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" class="form-control @error('email') is-invalid  @enderror"
                            value="{{ old('email' , $employee->email) }}" id="exampleInputEmail1">
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="company_id" class="form-select @error('company_id') is-invalid  @enderror"
                            ...
                            @foreach ($companies as $company)
                                <option value="{{ $company->id }}" @selected($company->id == old('company_id' , $employee->company_id))>{{ $company->name }}</option>
                            @endforeach
                        </select>
